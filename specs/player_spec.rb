...
describe "Player class" do
  ...
  describe "Player#draw_tiles(tile_bag)" do
    it "return ArgumentError if parameter is not an instance of TileBag" do
      proc {
        player = Scrabble::Player.new("Natalia")
        score = Scrabble::Scoring.new
        player.draw_tiles(score)
      }.must_raise ArgumentError
    end
    it "Returns an array with some tiles " do
      bag = Scrabble::TileBag.new
      player = Scrabble::Player.new("laura")
      player.draw_tiles(bag).class.must_equal Array
    end

    it "Length of returning tiles must be always 7 " do
      bag = Scrabble::TileBag.new
      player = Scrabble::Player.new("laura")
